  ...
  <div class="modal-box">
    ...
    <div class="modal-action flex flex-col">
      <form method="dialog" @submit.prevent="submitValorem">
        ...
        ref="inputRef"
        type="text" 
        :placeholder="placeholder ?? 'Inserte um texto'" 
        class="input input-bordered w-full w-full flex-1 input-primary mb-6" 
        v-model="inputValorem"
        />
        <div class="flex justify-end">
       
        <button @click="$emit('claudere')" class="btn">Cerrar</button>
...
<div v-if="aperta" class="fixed top-0 w-screen h-screen z-10 bg-black opacity-60"></div>
</template>
